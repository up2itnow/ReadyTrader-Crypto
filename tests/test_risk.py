...
def test_position_sizing(risk_guardian):
    ...
    result = risk_guardian.validate_trade('buy', 'BTC/USDT', 400.0, 10000.0, 0.0)
    assert result['allowed'] is True

    # Test unsafe trade
    # Portfolio: 10,000. Trade: 600 (6%) -> Blocked (>5%)
    result = risk_guardian.validate_trade('buy', 'BTC/USDT', 600.0, 10000.0, 0.0)
    assert result['allowed'] is False
    assert "Position size too large" in result['reason']

def test_falling_knife_protection(risk_guardian):
    # Test normal buy
    result = risk_guardian.validate_trade('buy', 'BTC/USDT', 100.0, 10000.0, 0.0)
    assert result['allowed'] is True

    # Test buy with bad sentiment
    result = risk_guardian.validate_trade('buy', 'BTC/USDT', 100.0, 10000.0, -0.6)
    assert result['allowed'] is False
    assert "Falling Knife" in result['reason']

    # Test SELL with bad sentiment (should be allowed, cutting losses)
    result = risk_guardian.validate_trade('sell', 'BTC/USDT', 100.0, 10000.0, -0.6)
    assert result['allowed'] is True

def test_daily_loss_limit(risk_guardian):
    # Tests Daily Loss Limit (5%)
    # Case 1: No loss -> Allowed
    result = risk_guardian.validate_trade('buy', 'BTC/USDT', 100.0, 10000.0, 0.0, daily_loss_pct=0.0)
    assert result['allowed'] is True
    
    # Case 2: Hit Limit (-5%) -> Blocked
    result = risk_guardian.validate_trade('buy', 'BTC/USDT', 100.0, 10000.0, 0.0, daily_loss_pct=-0.05)
    assert result['allowed'] is False
...
def test_max_drawdown_limit(risk_guardian):
    ...
    result = risk_guardian.validate_trade('buy', 'BTC/USDT', 100.0, 10000.0, 0.0, current_drawdown_pct=0.05)
    assert result['allowed'] is True
    
    # Case 2: Hit Limit (10%) -> Blocked
    result = risk_guardian.validate_trade('buy', 'BTC/USDT', 100.0, 10000.0, 0.0, current_drawdown_pct=0.10)
    assert result['allowed'] is False
...
def test_paper_engine_risk_metrics_nonzero(tmp_path):
    ...
    # Seed with stablecoin so equity starts > 0
    engine.deposit("agent_zero", "USDC", 1000.0)
    # Buy some BTC at $50k (this also sets BTC price cache)
    engine.execute_trade("agent_zero", "buy", "BTC/USDC", 0.01, 50000.0, "test")
